...
def subprocess_fn(rank, args, temp_dir):
    ...
    # Init torch_utils.
    sync_device = torch.device('mps' if torch.backends.mps.is_available() else 'cuda', rank) if args.num_gpus > 1 else None
    training_stats.init_multiprocessing(rank=rank, sync_device=sync_device)
    if rank != 0 or not args.verbose:
        custom_ops.verbosity = 'none'

    # Print network summary.
    device = torch.device('mps' if torch.backends.mps.is_available() else 'cuda', rank)
